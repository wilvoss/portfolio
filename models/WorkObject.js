...
var Works = [
  new WorkObject({
    ...
    caseStudyObjects: [
      ...
      new CaseStudyObject({
        html: '<h2>The challenge</h2><p>Initially, <i>Like Me?</i> was a proof-of-concept, web-based, prototype video game. While it functioned, it lacked polish and a compelling feature set. The goal was to bring this product to market as soon as possible. The core gameplay was great, but the UI was unbalanced, unintuitive, unhelpful, and somewhat unattractive. Specifically, users were unsure of where to look first, what certain icons meant, which information was useful, and how to play the game successfully without relying heavily on inference and trial and error. Features that could be added included score keeping, score sharing, and a daily call to action to name a few.</p><p>Here are what the four main screens (home, game play, game over, how to) looked like at the start of this journey:</p>',
      ...
      new CaseStudyObject({
        html: '<p>After three months of diligent work, the final product, now available on all major app stores, speaks for itself. Producing this game involved a ton of iteration, all of which was done in the web prototype. Instead of getting gameplay feedback from users and testers in the beginning, I worked with them to iron out the hierarchical kinks, visual anomalies, and confusing UI elements first, and then engaged with them to address gameplay issues.</p>',
      }),
      new CaseStudyObject({
        html: '<h3>Unpacking the process</h3><p>Progress began with UI improvements like tweaking overall spacing, sizing, colors, and UX flows while generating a positive feedback loop with players, fellow game designers and UX designers along the way. As well, I did this when I added features that required iconography, animation, visual assets, and content by sending out texts with comparison screenshots or by updating the prototype and asking folks for their opinions. Often I would grab an existing resource, test its viability and reception, then generate my own version, only to repeat this process until I was satisfied with the results.</p>',
      }),
      new CaseStudyObject({
